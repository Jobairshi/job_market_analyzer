...
export default function SkillGapPage() {
  ...
  const handleAnalyze = async () => {
    ...
    setError(null);
    try {
      const data = await api.aiSkillGap(resumeText, jobDesc);
      setResult(data);
    } catch (err: any) {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      <button
        ...
        disabled={loading || resumeText.length < 10 || jobDesc.length < 10}
              ...
              <div className="mt-4 space-y-3">
                {result.learning_path.map((item, idx) => {
                  ...
                  return (
                    ...
                    >
                      <span className={`rounded-full border px-2.5 py-0.5 text-xs font-semibold ${typeColor}`}>
                        {item.type}
                      </span>
                      <div>
