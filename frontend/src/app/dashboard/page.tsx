...
export default function DashboardPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <section className="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-5">
          {loading ? (
            ...
            <>
              ...
              <StatsCard
                title="Last 24 Hours"
                value={summary.jobsLast24h.toLocaleString()}
                subtitle="New jobs today"
                icon={<ClockIcon />}
                color="emerald"
              />
              <StatsCard
                title="Remote %"
                value={`${summary.remotePercentage}%`}
              ...
              <StatsCard
                title="Top Company"
                value={summary.topCompany}
                icon={<BuildingIcon />}
                color="amber"
              />
              <StatsCard
                title="Top Skill"
                value={summary.topSkill}
                icon={<SparklesIcon />}
                color="rose"
              />
            </>
        ...
        {/* Charts Row 1: Timeline + Skills */}
        <section className="mb-8 grid grid-cols-1 gap-6 lg:grid-cols-2">
          {loading ? (
            <>
              <JobsLineChartSkeleton />
              <SkillsBarChartSkeleton />
            </>
          ) : (
            <>
              <JobsLineChart data={timeline} />
              <SkillsBarChart data={skills} />
            </>
          )}
        </section>

        {/* Charts Row 2: Companies + Locations */}
        <section className="mb-8 grid grid-cols-1 gap-6 lg:grid-cols-2">
          {loading ? (
            <>
              <CompanyPieChartSkeleton />
              <LocationBarChartSkeleton />
            </>
          ) : (
            <>
              <CompanyPieChart data={companies} />
              <LocationBarChart data={locations} />
            </>
          )}
        </section>

        {/* Skill Cluster Graph (full width) */}
        <section className="mb-8">
          {loading ? (
            <SkillClusterGraphSkeleton />
          ) : clusters ? (
            <SkillClusterGraph data={clusters} />
          ) : null}
        </section>
